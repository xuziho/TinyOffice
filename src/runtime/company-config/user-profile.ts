...
export type UserProfile = { schema: "tinyoffice-user-profile"; version: 3; id: string; displayName: string; avatarSeed: string; initialized: boolean };
...
export async function loadUserProfile(input: { repoRoot: string; userId: string; fallbackDisplayName?: string }): Promise<UserProfile> {
  ...
  if (!postgres) throw new Error("User profile requires PostgreSQL.");
  try {
    const rows = await postgres.client.query<{ display_name: string; avatar_seed: string; profile_initialized_at: Date | null }>("SELECT display_name, avatar_seed, profile_initialized_at FROM user_profiles WHERE user_id = $1", [input.userId]);
    return { schema: "tinyoffice-user-profile", version: 3, id: input.userId, displayName: rows.rows[0]?.display_name ?? input.fallbackDisplayName ?? "", avatarSeed: rows.rows[0]?.avatar_seed ?? input.userId, initialized: Boolean(rows.rows[0]?.profile_initialized_at) };
  } finally { postgres.client.release(); await endCompanyPostgresPool(postgres.pool); }
}

export async function saveUserProfile(input: { repoRoot: string; userId: string; displayName: unknown; avatarSeed: unknown }): Promise<UserProfile> {
  ...
  try {
    ...
    await postgres.client.query(`INSERT INTO user_profiles (user_id, display_name, avatar_seed, profile_initialized_at, created_at, updated_at) VALUES ($1, $2, $3, now(), now(), now()) ON CONFLICT (user_id) DO UPDATE SET display_name = EXCLUDED.display_name, avatar_seed = EXCLUDED.avatar_seed, profile_initialized_at = COALESCE(user_profiles.profile_initialized_at, now()), updated_at = now()`, [input.userId, displayName, avatarSeed]);
    ...
    return { schema: "tinyoffice-user-profile", version: 3, id: input.userId, displayName, avatarSeed, initialized: true };
