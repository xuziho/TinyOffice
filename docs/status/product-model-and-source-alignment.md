...
| --- | --- | --- | --- |
| Company / Workspace | `companies`, `company_members`, explicit company-scoped APIs. | Current-session, workspace header context, and shadcn Company lifecycle surface exist. Ongoing System AI configuration is developer-gated and separate. | First version usable; login/real identity still needed. |
...
| Handoff / Single-ball Owner | Existing handoff/runtime semantics are present. | Not yet a formal standalone frontend flow. | Future Topic slice. |
...
| Runtime Dispatch | Chat turn dispatch wakes direct peer/mentioned/runtime target. | Visible through replies and presence. | First product chain works; failure/retry/status polish pending. |
