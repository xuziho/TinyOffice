...
Resource collection workspaces place creation beside the collection title: `New employee` belongs to Employees and `New company` belongs to Companies. Persistent success labels should not compete with the primary action; configuration surfaces show `Saving` or `Unsaved changes` when the user can act and remain quiet once saved. Normal lifecycle state should be inferred from its collection where possible, while exceptional states such as an inactive employee remain explicit.
