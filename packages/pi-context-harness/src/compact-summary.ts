...
export function buildBasicSummary(preparation: CompactionPreparation, maxChars: number): string {
	...
	return [
		...
		"- None explicitly identified in the compacted context.",
		"",
...
function buildLlmPrompt(preparation: CompactionPreparation, maxChars: number, omittedMessages = 0): string {
	return [
		"Create a detailed coding-agent continuity summary. Respond with markdown only and use exactly these sections:",
		"Current Goal, User Constraints And Preferences, Completed Work, Key Decisions, Files Read, Files Modified, Tool Results And External Evidence, Open Questions Or Blockers, Immediate Next Steps, Continuation Instruction.",
		"Be precise about file paths, commands, errors, user corrections, and pending work. Do not invent facts.",
		omittedMessages > 0 ? `\nOmission marker: ${omittedMessages} older messages were omitted from this summary attempt because the compaction prompt was too large. Preserve this fact in Tool Results And External Evidence or Open Questions when relevant.` : "",
