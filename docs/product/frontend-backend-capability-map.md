...
| Capability registry: `/api/companies/:companyId/capabilities` | keep: developer inspection | Developer Tools / Capabilities. | Read-only visibility into system-owned descriptions, schemas, scenes, confirmation policy, and guardrails. |
...
| Access policy, preview, request list, request resolution, and tool-call bridge under `/api/companies/:companyId/access` | keep: developer surface + runtime/tool contract | Developer Tools / Access, Chat foreground Access cards, PI tool guard bridge. | Access request approval belongs in Chat foreground cards; the Access page is for advanced policy tuning. |
| Doctor: `GET /api/companies/:companyId/doctor` | keep: developer surface | Developer Tools / Doctor. | Read-only system/config diagnostics only; not business task failure review. |
| Full-instance backup engine and internal UI routes | keep: developer surface + operator CLI | Developer Tools / Backup & Restore; `agentco backup`. | TinyOffice owns verified package and restore semantics. External automation owns schedule, remote storage, retention, and notifications. |
