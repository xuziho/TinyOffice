...
- `chat.process_trace.appended` invalidates the room Activity query only. It must not invalidate authoritative message queries by itself. Activity refreshes are coalesced to prevent dense tool evidence from producing an HTTP request per trace row.
...
The standalone shadcn web app subscribes to the current socket.io route and imports the shared `tinyoffice/realtime-contracts` type. Persisted-data events invalidate TanStack Query keys; HTTP clients then refetch authoritative data. Expensive employee-summary and Sessions invalidations are coalesced with an immediate leading refresh, a 250 ms window, and a trailing refresh. Streaming draft events stay in the run-state layer. Process trace append events refresh coalesced Activity queries under the selected room/source-message scope and do not refresh the employee summary. Final persisted Messages still reconcile through `chat.message.created` and the normal Message/Projection APIs.
