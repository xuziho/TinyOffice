...
Full password login, registration, session ownership, authorization, and delegation/impersonation policy remain future work. TinyOffice now has a minimal current-user session contract for product initialization: the session can return the current user without a Company, or the current user plus the active Company member when a Company context exists. Until full login exists, local runtime and smoke URLs may carry an explicit member viewer such as `memberId=xuziho`, but that is development-preview behavior only. Product implementation must not add new behavior that depends on URL identity parameters as login identity. Server deployments can be protected by an outer access layer such as VPN, reverse-proxy auth, or Cloudflare Access. Chat projection, read-state, write, create-entry, and realtime preview paths send that member identity directly through `viewerMemberId` or `actorMemberId`; they must not resolve the boss/member viewer through an internal employee API actor or local member-to-employee mapping.
