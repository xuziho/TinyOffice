...
export const capabilityRegistry = {
  ...
  capabilities: [
    ...
    },
    {
      id: "mcp.tools.list",
      category: "system",
      title: "List assigned MCP tools",
      description: "Discovers the MCP tools currently assigned to this employee, including each tool's input schema.",
      effect: "read",
      allowedScenes: ["chat_dm", "chat_channel", "work_run"],
      confirmationPolicy: { required: false, mode: "none" },
      useWhen: "Use when an external service or specialized tool may help. Call this before assuming an assigned MCP tool does or does not exist.",
      inputSchema: {
        type: "object",
        required: ["companyId"],
        additionalProperties: false,
      ...
      outputSchema: {
        ...
        properties: {
          ...
          unavailableConnections: { type: "array", items: { type: "object" } },
        },
      },
      notes: [
        "Only deployment connections assigned to the current Company or employee are returned.",
        "One unavailable connection does not hide tools discovered from other healthy assigned connections; inspect unavailableConnections when discovery is partial.",
        "Use the exact connectionId, tool name, and input schema returned here with mcp.tool.call.",
        "Assignment is the TinyOffice authority boundary; do not invent unassigned MCP tools or connection ids.",
      ],
    },
    {
      id: "mcp.tool.call",
      category: "system",
      title: "Call an assigned MCP tool",
      description: "Calls one MCP tool through a connection assigned to the current employee and records TinyOffice audit evidence.",
      effect: "execute",
      allowedScenes: ["chat_dm", "chat_channel", "work_run"],
      confirmationPolicy: { required: false, mode: "none" },
      useWhen: "Use after mcp.tools.list identifies an assigned tool that is relevant to the current request.",
      inputSchema: {
        type: "object",
        required: ["companyId", "connectionId", "toolName", "arguments"],
        additionalProperties: false,
        properties: {
          companyId: companyIdProperty,
          connectionId: { type: "string", description: "Exact connectionId returned by mcp.tools.list." },
          toolName: { type: "string", description: "Exact MCP tool name returned by mcp.tools.list." },
      ...
      notes: [
        "An assignment means the employee may use the connection; no extra generic business approval is added here.",
        "Unknown, disabled, or unassigned connections fail explicitly.",
        "Never place credentials in arguments when the connection can resolve them from host environment references.",
      ],
    },
    {
      id: "mcp.admin.describe",
      category: "system",
      title: "Describe MCP configuration",
      description: "Returns the deployment MCP definitions and current Company assignments without returning credential values.",
      effect: "read",
      allowedScenes: ["chat_dm", "chat_channel"],
      confirmationPolicy: { required: false, mode: "none" },
      useWhen: "Use when the operator asks to install, connect, assign, or troubleshoot an MCP server.",
      inputSchema: { type: "object", required: ["companyId"], additionalProperties: false, properties: { companyId: companyIdProperty } },
      outputSchema: { type: "object", properties: { schema: { type: "string" }, companyId: { type: "string" }, servers: { type: "array", items: { type: "object" } }, connections: { type: "array", items: { type: "object" } }, assignments: { type: "array", items: { type: "object" } } } },
      notes: [
        "Credential values are never returned; only host environment variable reference names and resolution state are visible.",
        "Inspect existing ids before proposing mcp.admin.configure so definitions are updated deliberately rather than duplicated.",
      ],
    },
    {
      id: "mcp.admin.configure",
      category: "system",
      title: "Configure and assign MCP",
      description: "Applies a confirmed MCP Server, Connection, or Assignment configuration through TinyOffice's provider-neutral boundary.",
      effect: "update",
      allowedScenes: ["chat_dm", "chat_channel"],
      confirmationPolicy: { required: true, mode: "operator", description: "Show the exact command or URL, environment-variable reference names, scope, and affected employees; call only after explicit operator confirmation." },
      useWhen: "Use after mcp.admin.describe, validating the requested MCP package or endpoint, presenting the exact proposal, and receiving explicit confirmation.",
      inputSchema: {
        type: "object", required: ["companyId", "operation", "configuration"], additionalProperties: false,
        properties: {
          companyId: companyIdProperty,
          operation: { type: "string", enum: ["save_server", "save_connection", "save_assignment"] },
          configuration: { type: "object", additionalProperties: true, description: "Configuration for the selected operation. Secret values are forbidden; use envRefs or headerRefs to name host environment variables." },
        },
      },
      outputSchema: { type: "object", properties: { schema: { type: "string" }, companyId: { type: "string" }, operation: { type: "string" }, state: { type: "object" } } },
      notes: [
        "Never place tokens, passwords, cookies, or API keys in configuration. Store only host environment variable names in envRefs or headerRefs.",
        "Installing a definition does not expose it to employees until a Company or Employee assignment is saved.",
        "Do not install arbitrary PI Marketplace extensions through this capability; it owns MCP definitions only.",
      ],
    },
    {
      id: "skill.list",
