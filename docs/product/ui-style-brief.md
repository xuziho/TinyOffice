...
Shared visual treatment does not make every page structurally identical. Sessions remains an evidence inspector; Employees and Company Skills remain configuration workspaces; Integrations remains an intake setup surface; System AI, Prompt, Access, Capabilities, and Doctor retain their technical workflows; Settings and Backup retain their existing product scope. The shared layer must never replace a route's fields, grouping, permissions, filters, actions, or runtime behavior.
