...
- Channel/Topic turns also produce a normal assistant reply. The model must call `handoff_topic_turn` exactly once with one `toId`. Returning the Topic ball to the user means selecting that user's participant id, not omitting the state action.
- `handoff_topic_turn` carries state only. It must not contain a visible reply `message` field.
- Every user message in a Channel Topic starts exactly one runtime path. A structured mention selects only its first runtime-capable member. With no structured mention, Runtime Dispatch deterministically selects one eligible runtime-capable Channel participant. Literal `@all` text uses this same no-mention route and is not a broadcast.
- A product member actor can wake a runtime-capable DM peer, the first explicitly mentioned employee, or the deterministic no-mention Topic receiver while remaining a member actor. Runtime Dispatch must not map the member actor into an employee.
...
- runtime employee replies may show compact per-turn token usage in the message meta line, using the linked Session evidence for the source message that triggered that reply. Repeated provider usage snapshots for the same model call count once; a real follow-up model call, including a Channel Handoff state-action repair, counts once in the same visible reply turn. Session Overview remains a whole-session total, so it is not expected to equal one Chat reply when the persistent session contains other turns. The reply block itself opens the matching Activity dock, and the dock labels the selected reply by sender and time. Token usage is a quiet scan aid, not a replacement for the detailed Sessions usage view. In a concrete Channel Topic, the Participants list marks only the current Topic holder with a quiet `Replying…` status and supplemental green activity dot; a cancel request changes that row to `Stopping…`. The marker follows the single-ball handoff and must not expose unrelated DM, Task, or background-work status as if it belonged to the current Topic.
...
- Channel/Topic runtime context uses progressive disclosure and has no user-visible context switch. Runtime receives explicit Topic context assembled from a System AI-maintained Topic summary, a raw-message window, the current triggering or handoff message marked inside that window, handoff candidates, and approved history lookup tools for older details. The trigger body is not appended again as a separate `User Message`. Handoff candidates are current Conversation participants excluding the member taking the current turn. On the first turn for a Topic runtime session, the newest 20 persisted Conversation messages are selected and rendered chronologically. On later turns for the same persistent runtime `sessionKey`, Runtime Dispatch reads the previous Topic context cursor from Session evidence and renders every raw message after that cursor without applying the initial 20-message cap. System AI summaries live on Conversation Topic state, compress older Topic history, and never replace persisted Conversation messages.
...
- Activity is shown only from real trace evidence projected by the backend; Chat Context keeps a stable Activity dock with an empty state until a running turn or employee reply is selected. During a Topic handoff, the dock keeps the last settled turn visible until the next receiver has real projected trace evidence, labels that temporary state `Previous reply`, then switches the sender label and activity rows together; it must not flash an empty or mismatched dock between receivers. Provider auto-retry/backoff is projected as visible Activity so a transient provider or network wait does not look like unexplained inactivity. When a concrete message room is reopened, the dock selects the latest employee reply that has explicit Activity evidence, so the user sees the same most recent run without manually reselecting it. Employee replies can expose Activity only when their runtime link carries an explicit source message id, so the dock shows that turn instead of the entire room. Channel/Topic `handoff_topic_turn` evidence is projected as a first-class Handoff activity, not a generic Tool call, and the collapsed Activity row shows the target participant when the trace includes one. Repeated tool operations appear as one compact named-tool summary in the bounded Chat dock; full operation evidence stays in Sessions. Detailed runtime progress remains in Sessions until a separate Trace destination is accepted.

- Employee message Markdown supports GitHub-flavored tables. Wide tables scroll horizontally inside the message body, table cells wrap long text, and inline links use the active theme's link color plus a visible underline so nested references remain distinguishable from surrounding text.
...
- explicit `@` mention chooses the first Channel/Topic receiver when the composer sends a matching structured `mentionedMemberIds` entry. Additional mentions remain visible context but do not create parallel dispatches. Display text is not a structured member mention. The composer may insert literal `@all`, but it must not add a synthetic member id, increment structured mention state, broadcast, or wake multiple employees.
- Channel/Topic runtime context must expose `Handoff candidates` with stable ids, display names, and product roles/responsibilities when available. Each candidate is rendered with explicit labeled fields such as `id="owner"; displayName="Xu Ziho"` so the stable id cannot be confused with a role or inferred from the display name. The list excludes the member taking the current turn; the model should choose `handoff_topic_turn.toId` from those candidates using the current topic, roles, and recent messages. The frontend and prompt context must not split candidates into human-vs-AI product categories. Unsupported Conversation kinds fail closed at runtime dispatch; TinyOffice must not silently map them into a shared-room prompt mode.
