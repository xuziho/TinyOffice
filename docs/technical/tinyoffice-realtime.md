...
type TinyOfficeRealtimeEventPayload =
  ...
  | { type: "chat.activity.observed"; companyId: string; conversationId: string; roomId: string; runId: string; sourceMessageId: string; targetMemberId: string; sessionKey?: string; sequenceInRun: number; activity: RuntimeActivity }
  | { type: "chat.activity.persisted"; companyId: string; conversationId: string; roomId: string; runId: string; sourceMessageId: string; targetMemberId: string; persistedThroughSequence: number }
...
- Runtime dispatch publishes `chat.activity.observed` immediately when new process evidence changes the backend-owned Activity projection. This event does not wait for PostgreSQL.
- The evidence writer publishes `chat.activity.persisted` after a batch becomes durable. Its watermark is used for reconciliation, not as a trigger to refetch every row.
...
- Frontend Activity rendering belongs to the stable Chat Context dock, not the Chat message stream. A room can contain multiple completed or running activities at the same time, especially during Channel handoff. The frontend reads the backend Activity projection for the selected active run or selected employee reply, using explicit `sourceMessageId` metadata rather than room-wide trace mixing. Employee message headers may own the navigation action that selects their turn, but the Activity content still renders in Context. The frontend applies backend-projected Activity upserts as a realtime overlay and reconciles that overlay with the persisted REST snapshot.
- Runtime streaming can publish `chat.reply.delta` and `chat.reply.snapshot`. These events are ephemeral draft output for the `runId`; they are not final persisted Messages and must not be treated as database truth. They update the run-state draft layer only. They must not invalidate authoritative message queries by themselves. The first non-empty text delta is emitted immediately; later text is coalesced on a 50 ms timer, including a trailing flush when the provider pauses, and completion forces out any remainder.
- `chat.activity.observed` updates local Activity without invalidating Message, Projection, employee-status, Session, or Activity queries. Durable Activity is reconciled on reconnect, sequence gaps, selection changes, and run termination.
- For a Chat runtime turn, the first real `chat.activity.observed` event for a `runId` must arrive before the first visible reply event (`chat.reply.delta` or `chat.reply.snapshot`) for that same `runId`. Neither event waits for Process Trace persistence.
...
The standalone shadcn web app subscribes to the current socket.io route and imports the shared `tinyoffice/realtime-contracts` type. Persisted business-data events invalidate TanStack Query keys; HTTP clients then refetch authoritative data. Expensive employee-summary and Sessions invalidations are coalesced with an immediate leading refresh, a 250 ms window, and a trailing refresh. Streaming drafts and backend-projected `chat.activity.observed` upserts stay in bounded in-memory overlays. Process Trace persistence publishes a watermark instead of forcing an Activity or employee-summary refetch for every evidence row. Final persisted Messages still reconcile through `chat.message.created` and the normal Message/Projection APIs.
